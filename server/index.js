...
// Initialize Twilio (you'll need to set these environment variables)
const twilioClient = process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN 
  ? twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN)
  : null;

// Initialize SendGrid (you'll need to set this environment variable)
...
async function sendNotification(recipient, claimCode, amount, message, network = 'testnet') {
  ...
  try {
    if (isEmail) {
      ...
      if (!process.env.SENDGRID_API_KEY) {
        console.log(`Would send email to ${recipient}: ${notificationMessage}`);
        return { success: true, method: 'email_simulation' };
      }

      const msg = {
        to: recipient,
        from: process.env.SENDGRID_FROM_EMAIL || 'noreply@randcash.app',
      ...
      // Send SMS via Twilio
      if (!twilioClient) {
        console.log(`Would send SMS to ${recipient}: ${notificationMessage}`);
...
app.listen(PORT, () => {
  ...
  console.log(`Twilio SMS: ${twilioClient ? 'Configured' : 'Not configured (will simulate)'}`);
  console.log(`SendGrid Email: ${process.env.SENDGRID_API_KEY ? 'Configured' : 'Not configured (will simulate)'}`);
});